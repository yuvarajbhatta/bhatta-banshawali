...
- **Server-driven, generation-scoped payloads**: `GET /api/v1/family-tree` accepts optional `minGeneration`/`maxGeneration` params (both null = everyone, unchanged for `/family`'s unwindowed fetch); `/tree` defaults to a bounded window rather than fetching and rendering the whole graph. **Confirmed necessary, then built and verified (2026-07-31)**: this was never implemented, and benchmarking the as-built (whole-graph) approach showed exactly the super-linear degradation this section predicted (500→2,000 people: Dagre layout time ~0.9s→~18s, real render time ~5s→~30-35s). Fixed the same day with generation-windowing (`useTreeWindow.ts` + the backend params above) — re-benchmarked at ~1.5s for the same 2,000-person dataset. True viewport/pan-position-driven fetching (finer-grained than a generation window) and branch-based (non-generation) lazy expand/collapse remain unbuilt, explicitly deferred as separate follow-ups — the generation-window approximation was sufficient to close this gate. See `docs/08-implementation-roadmap.md` Phase 5 for full results.
- **Benchmark before committing**: load-test the chosen approach against synthetic datasets of 500, 2,000, and (if plausible for this family's actual size) 10,000 people before finalizing the lazy-loading page-size defaults. Done — see above.
